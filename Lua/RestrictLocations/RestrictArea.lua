-- These are the area ids that will be restricted add more area IDs as needed currently set to goldshire.
local restrictedAreaIds = {87}
...
local function OnUpdateArea(event, player, newZone, newArea)
    -- If player is a gm return and ignore the script.
    if (player:GetGMRank() >= gmRank) then
        return
    end

    -- Loop through all restricted area IDs
    for _, restrictedAreaId in ipairs(restrictedAreaIds) do
        if (newArea == restrictedAreaId) then
...
RegisterPlayerEvent(27, OnUpdateArea)